...
skiplogger = skipLogger(desktop, "Skipped Files")
...
def keyLoad(): #takes information gathered by findKeyfile and prints a confirmation message to screen, terminating if incorrect.
	...
	if str(importResult.fingerprints[0]) != expectedFP:
		...
		if confirmation == "OK":
			...
			with open("tapestry.cfg", "w") as cfg: #TODO detest
				config.write(cfg)
		else:
		...
		for file in files:
			if file.endswith(".tap") == True: 
				...
				with open(a+"/"+file, "rb") as k:
					...
					baz = gpg.decrypt_file(k, output=outputTGT, always_trust=True, passphrase=secret)
...
def verifyBlock(): #Performs a verification test of the target block
	...
	if verified.trust_level is not None and verified.trust_level >= verified.TRUST_FULLY:
		...
		print("The signature is neither fully nor ultimately trusted. Continue?")
		go = raw_input("y/n?")
		if go:
			valid = True
		else:
