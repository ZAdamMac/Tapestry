#! /usr/bin/python3.6
# Unit tests.old relating to Tapestry. Current as of 2.0.
...
"""
    ...
    namespace.logger = fw.SimpleLogger("Logs", namespace.filename, "unit-tests.old")
    namespace.failed_once = False  # Bool to show if any tests.old have failed.
...
def import_for_keys(ns):
    ...
    level of minimal or higher for tests.old to work, and waits for them to do so.
    """
    found_enc_key = True
    found_sig_key = True
    ...
    if found_enc_key and found_sig_key:
        ...
        if not found_enc_key:
            ...
            # trusted
            print("Done")
        if not found_sig_key:  # Without this sig tests.old fail.
...
def test_config_compliance(ns):
    ...
    tapestry-tests.old.cfg. See above for warnings.""")
...
def test_riff_compliance(ns):
    """This test compares the known-good RIFF (namespace.goodRIFF) to an RIFF
        generated by the runtime tests.old and extracted in the integrity test
...
def runtime(home):
    os.chdir(home)
    print("Now beginning the unit-tests.old for tapestry.")
    ...
    #data = test_verification_good(data)  # These are covered in integration tests.old.
    #data = test_verification_bad(data)
    if data.failed_once:
        print("Some tests.old were failed. Please try again.")
        data.logger.save()
    else:
        print("All unit tests.old passed. Please try again.")
        data.logger.save()
